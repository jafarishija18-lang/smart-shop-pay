DROP FUNCTION IF EXISTS public.process_checkout(text, text, jsonb);
DROP FUNCTION IF EXISTS public.register_student(text, text, text, text, numeric);
...
CREATE INDEX idx_student_credentials_student ON public.student_credentials(student_id);
ALTER TABLE public.student_credentials ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Admins manage student credentials"
...
CREATE OR REPLACE FUNCTION public.register_student(_student_code text, _name text, _email text, _initial_balance numeric)
RETURNS uuid
...
BEGIN
  ...
  INSERT INTO public.students (student_code, name, email, balance, initial_balance)
    VALUES (_student_code, _name, _email, _initial_balance, _initial_balance)
